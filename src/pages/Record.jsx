...
import { progressSaveApi1, progressSaveApi2 } from '../components/API/StorySave';
...
export default function Record(props) {
    ...
    useEffect(() => {
        (async () => {
          ...
          {
            try {
                const response = await axios.get(`${url}stories/${state.item.storyId}`,
                    {
                        headers: {
                            'Accept': '*/*',
                            'Authorization': `Bearer ${token}`,
    ...
    const handleProgressSubmit = async() =>{
        if(stroyId) //storyId가 존재하는 경우 , 캘린더 -> 작성전(storyId존재) 스토리 저장 (성공)
        {
            try{
        ...
        else //storyId가 존재하지 않는 경우, 작성하기 버튼 -> 새로운 스토리(storyId존재x) 저장 (실패)
        {
            //alert("작성하기 버튼으로 들어옴") 
            try{
    ...
    const saveStory = () =>{
        if(stroyId)
        {
            try{
                //스토리 아이디 존재
                createStory1(stroyId,id,data,title,viewingTime,companion,genre1,genre2,genre3,satisfactionLevel,weather,isOpen,year,month,date);
        ...
        else 
        {
            try{
                //스토리 아이디 존재x
                createStory2(id,data,title,viewingTime,companion,genre1,genre2,genre3,satisfactionLevel,weather,isOpen,year,month,date);
                //navigate(`/mystory`, { state: { markEvent } }) 
                setIsNotifyModal(true)
            }catch (error) {
            console.log(error.response.data);
...
return (
    <div>
        ...
        <StoryRecord>
            ...
            <OpenSelectButton>
                <button onClick={ClickedOpen} style={dateBoxColor}>공개</button>
